...
Поле `content_title` содержит отдельное название контента страницы (с учетом выбранного языка).
...
К блоку любого типа можно прикрепить файл через поле `file`. API возвращает как само поле
`file`, так и абсолютную ссылку для фронтенда в `file_url`.
...
### Новости
- `GET /api/news/` — карточки активных новостей: фото, заголовок и краткое описание.
- `GET /api/news/{slug}/` — выбранная новость с полным описанием и тремя фотографиями.
- `POST /api/news/` — добавить новость.
- `PUT/PATCH /api/news/{slug}/` — обновить новость.
- `DELETE /api/news/{slug}/` — удалить новость.

Фильтры и сортировка:
- `?search=текст` — поиск по заголовку и описаниям.
- `?ordering=order`, `?ordering=-published_at` или `?ordering=-created_at`.
...
- **03. Новости** — карточки новостей и содержимое детальной страницы (описание и три фото).
