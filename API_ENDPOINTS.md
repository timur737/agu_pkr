...
- `photo_text` — фото + description.
  Для карточек руководства и почетных профессоров доступны отдельные поля
  `short_information`, `contacts` и `email`. Поле `url` можно использовать для
  ссылки на сайт в карточках партнеров.
